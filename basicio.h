...
//ハードウェアリセットと同義
#define reset()                     vAHI_SwReset()

extern uint64_t wakeTimer_getSleptTime();
...
//ウェイクタイマーの現在のカウント値を返します。uint64_t 型であることに注意してください
...
//遅延でない、ハードウェア割り込み
extern bool_t dio_attachFastCallback(uint32_t risingEdge, uint32_t fallingEdge, void (*func)(uint32_t u32DioBitmap));
extern bool_t dio_detachFastCallback();
...
extern void i2cs_enable(uint8_t u16Address, bool_t b10BitAddress, bool_t bUseSecondPin, void (*prepareFunc)(uint8_t), void (*receivedFunc)(uint8_t*, uint16_t), uint8_t *pReceiveBuffer, uint16_t receiveBufferSize);
...
extern int16_t radio_write(uint32_t u32DestAddr, uint8_t u8DataType, const uint8_t *pu8Data, uint8_t u8Length);
...
#define TWELITE_ARIA_WDT_PIN        13
