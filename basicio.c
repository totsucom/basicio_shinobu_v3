...
static uint64_t sleepTimerSetCount;
static uint64_t sleepTimerSetCalibValue;
static uint64_t sleptTime;

//前回のスリープ時間を返す。無効な場合は0を返す。
//DIOウェイクの場合でもタイマー残数から計算されるので正しい値を返す
//この関数を有効にするにはタイマースリープでかつRAM=ONの必要がある
uint64_t wakeTimer_getSleptTime() {
    return sleptTime;
}
...
bool_t sleepCalibratedTimer(uint64_t milliSeconds, bool_t bRAMPower, uint32_t calibValue) {
    ...
    sleepTimerSetCount = count;             //起床時にスリープ時間を計算するために記憶
    sleepTimerSetCalibValue = calibValue;
...
//割り込みルーチンのポインタを保持（ハードウェア割り込み）
void (*dioFastCallbackFunction)(uint32_t);  //コールバック関数のポインタ

//遅延でない、ハードウェア割り込み
//この関数を登録するとdio_attachCallback()で登録された遅延割り込みは無視されるので注意
//risingEdgeとfallingEdgeはビットb0-b19をDIO0-DIO19に対応させて、該当するビットをONすること
bool_t dio_attachFastCallback(uint32_t risingEdge, uint32_t fallingEdge, void (*func)(uint32_t u32DioBitmap)) {
    if (risingEdge == 0 && fallingEdge == 0) {
        return dio_detachFastCallback();
    }

    if (risingEdge & fallingEdge) return FALSE; //RISING FALLING はいずれか１つ
    if ((risingEdge & 0xfffff) != risingEdge || (fallingEdge & 0xfffff) != fallingEdge || func == NULL) return FALSE;

    //処理ルーチンのポインタを新規登録
    dioFastCallbackFunction = func;

    //割り込みを有効にする
    vAHI_DioInterruptEnable(risingEdge | fallingEdge, 0);
    vAHI_DioInterruptEdge(risingEdge, fallingEdge);
    return TRUE;
}

bool_t dio_detachFastCallback() {
    dioFastCallbackFunction = NULL;
    //割り込みを無効にする
    vAHI_DioInterruptEnable(0, 0xfffff);
    return TRUE;
}
...
static uint16_t adcConvertTemp(uint16_t src) {
    ...
    return 250 - (resultValue - (730 * 1024)) / 170;
...
static uint8_t i2cI2csInitFlag; //I2CINITSTATE
#endif
...
void i2c_enable(I2CCLOCKS clock, bool_t bUseSecondPin) {
    if (i2cI2csInitFlag == I2CINIT_SLAVE) vAHI_SiSlaveDisable();
    vAHI_SiSetLocation(bUseSecondPin);
    vAHI_SiMasterConfigure(TRUE, FALSE, clock);
    i2cI2csInitFlag = I2CINIT_MASTER;
}

void i2c_disable() {
    vAHI_SiMasterDisable();
    if (i2cI2csInitFlag == I2CINIT_MASTER) i2cI2csInitFlag = I2CINIT_NONE;
}
...
static uint8_t *i2cspau8MWBuffer;       //受信データの先頭ポインタ by user
static uint16_t i2csu16MWBufferSize;    //受信データ長 by user
static uint16_t i2csu16MWSize;          //受信したデータ長

static const uint8_t *i2cspau8MRBuffer; //送信データの先頭ポインタ by user
static uint8_t i2csu8MRBufferSize;      //送信データ長 by user
...
static void (*i2csMWCallbackFunction)(uint8_t *, uint16_t);

void i2cs_enable(uint8_t u16Address, bool_t b10BitAddress, bool_t bUseSecondPin,
    void (*prepareFunc)(uint8_t),
    //void (*receivedFunc)(uint8_t*, uint8_t),
    void (*receivedFunc)(uint8_t*, uint16_t), uint8_t *pReceiveBuffer, uint16_t receiveBufferSize) {
    
    if (i2cI2csInitFlag == I2CINIT_MASTER) vAHI_SiMasterDisable();

    i2csMRCallbackFunction = prepareFunc;
    i2csMWCallbackFunction = receivedFunc;
    i2cspau8MWBuffer = pReceiveBuffer;
    i2csu16MWBufferSize = receiveBufferSize;
...
*/
    i2cI2csInitFlag = I2CINIT_SLAVE;
}

void i2cs_disable() {
    vAHI_SiSlaveDisable();
    if (i2cI2csInitFlag == I2CINIT_SLAVE) i2cI2csInitFlag = I2CINIT_NONE;
}
...
int16_t radio_write(uint32_t u32DestAddr, uint8_t u8DataType, const uint8_t *pu8Data, uint8_t u8Length)
{
...
void resetVars()
{
    ...
    //i2csu8MWIndex[0] = 0;
    //i2csu8MWIndex[1] = 0;
    i2cspau8MWBuffer = NULL;
    i2csu16MWBufferSize = 0;
    i2csu16MWSize = 0;
...
void cbAppWarmStart(bool_t bAfterAhiInit)
{
	...
	if (!bAfterAhiInit) {
        ...
        if (wakeTimer_isRunning() && sleepTimerSetCalibValue != 0) {
            uint64_t wt = wakeTimer_readCount();    //スリープに使用するタイマーはカウントダウン
            if (wt & 0x10000000000) wt = 0;        //完全にカウントしきっている
            sleptTime = (sleepTimerSetCount - wt) * sleepTimerSetCalibValue / 320000;
            sleptTime += 30;//適当
        }
...
/*#ifdef USE_I2S
    case E_AHI_DEVICE_SI:   //I2Cマスタースレーブ共通
        if (i2cI2csInitFlag == I2CINIT_SLAVE) {
            //I2C スレーブ
...
uint8_t cbToCoNet_u8HwInt(uint32_t u32DeviceId, uint32_t u32ItemBitmap)
{
...
#ifdef USE_I2CS
    case E_AHI_DEVICE_SI: //I2Cマスタースレーブ共通
        if (i2cI2csInitFlag == I2CINIT_SLAVE && u32ItemBitmap & 0x1d) {
            //I2C スレーブ割り込み
            ...
            if (u32ItemBitmap & 1) {//E_AHI_SIS_DATA_RR_MASK
                ...
                if (i2csu8LastInt == 1 || i2csu8LastInt == 8 || i2csu8LastInt == 16) {
                    ...
                    //遅延やめて、ここで呼び出す
                    if (i2csMRCallbackFunction != NULL) {
                        //マスターが読み出すかもしれないデータを準備
                        (*i2csMRCallbackFunction)(i2csu8cmd);
                    }                    
                    //bDone = TRUE;
                    ...
                    if (i2csbNewCmd) {
                        ...
                        i2csu16MWSize = 0;
                        if (i2cspau8MWBuffer && i2csu16MWSize < i2csu16MWBufferSize) {
                            *(i2cspau8MWBuffer + i2csu16MWSize) = i2csu8cmd;
                            ++i2csu16MWSize;
                        }
                    }
                    //データ保存
                    ...
                    if (i2cspau8MWBuffer && i2csu16MWSize < i2csu16MWBufferSize) {
                        *(i2cspau8MWBuffer + i2csu16MWSize) = c;
                        ++i2csu16MWSize;
                    }
                }
                i2csu8LastInt = 4;
                ...
                if (i2csu8LastInt == 4) {// && i2csu8MWIndex[i2csu8MWBufferSelect] > 0) {
                    ...
                    //遅延やめて、ここで呼び出す
                    if (i2csMWCallbackFunction != NULL) {
                        //マスターが書き込んだデータを処理
                        //(*i2csMWCallbackFunction)(i2csau8MWBuffer[i2csu8MWBufferSelect], i2csu8MWIndex[i2csu8MWBufferSelect]);
                        (*i2csMWCallbackFunction)(i2cspau8MWBuffer, i2csu16MWSize);
                        i2csu16MWSize = 0;
                    }
                    //bDone = TRUE;
                }
    ...
    case E_AHI_DEVICE_SYSCTRL:
#ifdef USE_DIO
        if ((u32ItemBitmap & 0xfffff) && dioFastCallbackFunction) {
            //DIO割り込み
            (*dioFastCallbackFunction)(u32ItemBitmap);
            return TRUE;
        }
#endif
#ifdef USE_PC
...
void sht40_begin() {
    //測定開始
    i2c_writeByteOnly(SHT40_ADDR, SHT40_TEMP_HUM_H_PREC_CMD);
}

//sht40_begin()をsetup()で呼び出した場合、
//loop()内2サイクル(8ms)で測定は終了している
void sht40_readData(int16_t *pTemp, int16_t *pHumi) {
    //10倍した値で温度湿度を計算
    uint8_t buf[6];
    i2c_readOnly(SHT40_ADDR, buf, 6);

    uint16_t w = (buf[0] << 8) + buf[1];
    int32_t t = w * 1750;
    *pTemp = (int16_t)(t / 65535 - 450);

    w = (buf[3] << 8) + buf[4];
    int32_t h = w * 1250;
    *pHumi = (int16_t)(h / 65535 - 60);
}

#endif //USE_I2C_SHT40


#ifdef USE_I2C_ADS1115

void beginADS1X15(uint16_t config) {
    uint8_t config_data[3];
    config_data[0] = 0x01;          //Points to Config reg
    config_data[1] = config >> 8;   //MSB of Config reg
    config_data[2] = config & 255;  //LSB of config reg
    i2c_writeOnly(0x48, config_data, sizeof(config_data));

    uint8_t lo_thresh_data[] = {
        0x02,       //Points to Lo Thresh reg
        0b00000000, //MSB of Lo Thresh reg
        0b00000000  //LSB of Lo Thresh reg
    };
    i2c_writeOnly(0x48, lo_thresh_data, sizeof(lo_thresh_data));

    uint8_t hi_thresh_data[] = {
        0x03,       //Points to Hi Thresh reg
        0b11111111, //MSB of Lo Thresh reg
        0b11111111  //LSB of Lo Thresh reg
    };
    i2c_writeOnly(0x48, hi_thresh_data, sizeof(hi_thresh_data));
}

bool_t isADS1X15Busy() {
    int16_t value;
    i2c_read(0x48, 0x01, (uint8_t *)&value, 2);    //Points to Config reg
    return (value & 0x8000) == 0;
}

//x10000 の値で返す
int32_t readADS1115(uint16_t config) {
    int16_t value;
    i2c_read(0x48, 0x00, (uint8_t *)&value, 2);    //Points to Conversion reg
    int32_t v32 = value;
    switch (config & 0xe00) {
    case ADS1X15_REG_CONFIG_PGA_6_144V: ///< +/-6.144V range = Gain 2/3
        return (v32 * 61440) >> 15;
    case ADS1X15_REG_CONFIG_PGA_4_096V: ///< +/-4.096V range = Gain 1
        return (v32 * 10) >> 3;
    case ADS1X15_REG_CONFIG_PGA_2_048V: ///< +/-2.048V range = Gain 2 (default)
        return (v32 * 10) >> 4;
    case ADS1X15_REG_CONFIG_PGA_1_024V: ///< +/-1.024V range = Gain 4
        return (v32 * 10) >> 5;
    case ADS1X15_REG_CONFIG_PGA_0_512V: ///< +/-0.512V range = Gain 8
        return (v32 * 10) >> 6;
    case ADS1X15_REG_CONFIG_PGA_0_256V: ///< +/-0.256V range = Gain 16
        return (v32 * 10) >> 7;
    }
    return 0;
}
...
//x10000 の値で返す
int32_t readADS1015(uint16_t config) {
    int16_t value;
    i2c_read(0x48, 0x00, (uint8_t *)&value, 2);    //Points to Conversion reg
    int32_t v32 = value >> 4;
    switch (config & 0xe00) {
